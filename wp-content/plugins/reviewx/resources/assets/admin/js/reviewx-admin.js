(function( $ ) {
	...
	var RX_Admin = {
		...
		onLoadSettings : function ( type = 'option' ) {
			...
			if($("#rx_"+type+"_allow_profile_img").is(':checked')){
				let allowed_image = "Enabled";
				$("#allow_profile_img").text(allowed_image);
			} else {
				let allowed_image = "No";
				$("#allow_profile_img").text(allowed_image);
			}
			...
			if($("#rx_"+type+"_allow_location").is(':checked')){
				let allowed_video = "Yes";
				$("#action_dispaly_location").text(allowed_video);
			} else {
				let allowed_video = "No";
				$("#action_dispaly_location").text(allowed_video);
			}
	...
	*/
	...
	const prevBtn = $("form .previous-btn");  
	const form = $("#rvx-export-form");

	// default select field for review import
	$('#comment_ID, #comment_author, #comment_author_email, #comment_date, #comment_content, #post_title, #rating, #reviewx_title').attr('checked', true);
	...
	form.on("submit", (e) => {  
		e.preventDefault();
		let exportFormFilterValue = [];
		let selectedColumnName = [];
		let modifiedColumnName = [];
		
		$("#rvx-export-form input[type='number'], #rvx-export-form input[type='date'],  #rvx-export-form select").each(function() {
			exportFormFilterValue.push($(this).val());
		});

		$("#rvx-export-form input[type='checkbox']:checked").each(function() {
			selectedColumnName.push($(this).val());
		});

		selectedColumnName.forEach(function(index){
			let columnFiledValue = $(`#${index}`)
			modifiedColumnName.push(columnFiledValue.val());
		})

		console.log(exportFormFilterValue);
		
		//call export ajax function
		export_ajax( exportFormFilterValue, selectedColumnName, modifiedColumnName );
	}); 
	...
	$(document).on('click', '#rvx-select-all-column', function(){
		$('input:checkbox').not(this).prop('checked', this.checked);
	})

	// export from submit ajax
	function export_ajax( exportFormFilterValue, selectedColumnName, modifiedColumnName ) {
		Swal.fire({
			onBeforeOpen: () => {
				Swal.showLoading()
			},
			title: 'Please Wait !',
			html: '<div class="rx-email-loader-html"> Review Exporting...</div>',
			allowOutsideClick: false,
			customClass: 'rx-mail-loader',
		});
		$.ajax({
			...
			url: ajaxurl,
			data: {
				action: 'rx_export_review',
				filterValue : exportFormFilterValue,
				selectedColumns : selectedColumnName,
				changeColumnName : modifiedColumnName,
			},
			success: function(res) {
				Swal.fire({
					title: 'CSV Generated!',
