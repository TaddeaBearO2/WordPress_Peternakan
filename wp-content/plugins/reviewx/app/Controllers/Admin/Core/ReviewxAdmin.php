    ...
    class ReviewxAdmin extends Controller
    {
        ...
        */
        ...
        public function rx_email_notification() {
            ...
            $selected_rating    = trim($_POST['rating']);
        ...
        public function rx_export_review() {
            ...
            $filter_field       = $_POST['filterValue'];
            $selected_column    = $_POST['selectedColumns'];
            $change_column      = $_POST['changeColumnName'];

            $export_limit       = intval( $filter_field[0] );
            $skip_export_row    = intval( $filter_field[1] );
            $start_date         = strval( $filter_field[2] );
            $end_date           = strval( date( 'Y-m-d', strtotime( $filter_field[3] . '+1 day' ) ) );
            $product_name       = strval( $filter_field[4] );
            $selected_rating    = $filter_field[5];
            $review_status      = $filter_field[6];
            $review_sort        = $filter_field[7];
            $review_sort_by     = $filter_field[8];

            $first_commnet_date = json_decode( json_encode($wpdb->get_results( "SELECT 
                    comment_date FROM {$wpdb->prefix}comments 
            ...
            if( $product_name == 'all' ){
                $sql_query = "SELECT $sql_column_name FROM {$wpdb->prefix}comments AS comments
                    INNER JOIN {$wpdb->prefix}posts AS post 
                    ON post.ID = comments.comment_post_ID 
                    WHERE (comments.comment_type = 'review' AND comments.comment_parent = 0
                        AND comments.comment_approved = '$review_status'
                        AND (comments.comment_date_gmt BETWEEN '$start_date' AND '$end_date'))
                    ORDER BY $review_sort $review_sort_by LIMIT $skip_export_row, $export_limit";
            } else {
                $sql_query = "SELECT $sql_column_name FROM {$wpdb->prefix}comments AS comments
                        INNER JOIN {$wpdb->prefix}posts AS post 
                        ON post.ID = comments.comment_post_ID 
                        WHERE (comments.comment_type = 'review' AND comments.comment_parent = 0
                            AND comments.comment_approved = '$review_status'
                            AND (comments.comment_date_gmt BETWEEN '$start_date' AND '$end_date'))
                            AND post.post_title= '$product_name'
                        ORDER BY $review_sort $review_sort_by LIMIT $skip_export_row, $export_limit";
            }
